...
def _check_base_files(db_dir):
    ...
    if not (os.path.isfile(db_dir+'/tessellation.shp') or os.path.isfile(db_dir+'/tessellation.geojson')):
        raise ValueError(
            'Tessellation is missing! There must be a file named tessellation.shp or tessellation.geojson')
    ...
    if not (os.path.isfile(db_dir+'/flows.csv')):
        raise ValueError(
            'Flows are missing! There must be a file named flows.csv')
...
def _compute_support_files(db_dir, tile_id_column, tile_geometry, oa_id_column, oa_geometry, flow_origin_column, flow_destination_column, flow_flows_column):
    ...
    try:
        tessellation = geopandas.read_file(
            db_dir+'/tessellation.shp', dtype={tile_id_column: str})
    except:
        tessellation = geopandas.read_file(
            db_dir+'/tessellation.geojson', dtype={tile_id_column: str})
    tessellation = tessellation[[tile_id_column, tile_geometry]]
    print('Reading output areas....')
    try:
        output_areas = geopandas.read_file(
            db_dir+'/output_areas.shp', dtype={oa_id_column: str})
    except:
        output_areas = geopandas.read_file(
            db_dir+'/output_areas.geojson', dtype={oa_id_column: str})
    ...
    output_areas['ctrs'] = '[' + \
        output_areas['x'].astype(str) + ',' + \
        output_areas['y'].astype(str) + ']'

    temp_out = output_areas[[oa_id_column, 'ctrs', 'area_km2']]
    temp_out.rename(columns={oa_id_column: 'geo_code',
                    'ctrs': 'centroid'}, inplace=True)
    ...
    mapping = geopandas.sjoin(
        output_areas, tessellation, how="inner", op="within")
    try:
        mapping.drop(columns=['index_right'], inplace=True)
    except:
        pass

    flows = pd.read_csv(db_dir+'/flows.csv', dtype={
                        flow_origin_column: str, flow_destination_column: str, flow_flows_column: int})
    flows = flows[[flow_origin_column,
                   flow_destination_column, flow_flows_column]]

    flows.rename(columns={flow_origin_column: 'residence',
                 flow_destination_column: 'workplace', flow_flows_column: 'commuters'}, inplace=True)
    ...
    for i, row in mapping.iterrows():
        if row[tile_id_column] not in tileid2oa2handmade_features:
            ...
            tileid2oa2handmade_features[row[tile_id_column]
                                        ][row[oa_id_column]] = dict()
        else:
            tileid2oa2handmade_features[row[tile_id_column]
                                        ][row[oa_id_column]] = dict()
    for i, row in features.iterrows():
        for item in zip(list(row.keys()), row.values):
            tileid2oa2handmade_features[row[tile_id_column]][item[0]] = [
                item[1]]
...
def tessellation_definition(db_dir, name, size):
    if not (os.path.isfile(db_dir+'/tessellation.shp') or os.path.isfile(db_dir+'/tessellation.geojson')):
        tessellation = tilers.tiler.get(
            "squared", base_shape=name, meters=size)
...
def load_data(db_dir, tile_id_column, tile_geometry, oa_id_column, oa_geometry, flow_origin_column, flow_destination_column, flow_flows_column):
    ...
    # oa_gdf
    oa_gdf = pd.read_csv(db_dir + '/processed/oa_gdf.csv.gz',
                         dtype={'geo_code': 'str'})
...
def load_model(fname, oa2centroid, oa2features, oa2pop, device, dim_s=1,
    ...
    model = ffnn.NN_MultinomialRegression(
        dim_s, dim_hidden, 'deepgravity',  dropout_p=dropout_p, device=device)
...
def instantiate_model(oa2centroid, oa2features, oa2pop, dim_input, device=torch.device("cpu"), dim_hidden=256, lr=5e-6, momentum=0.9, dropout_p=0.0, verbose=False):
    ...
    model = ffnn.NN_MultinomialRegression(
        dim_input, dim_hidden,  'deepgravity', dropout_p=dropout_p, device=device)

    return model


# Calculates distance between 2 points and accounts for curvature of the earth (Haversine Distance)
def earth_distance(lat_lng1, lat_lng2):
    ...
    ds = 2 * asin(sqrt(sin(dlat/2.0) ** 2 + cos(lat1)
                  * cos(lat2) * sin(dlng/2.0) ** 2))
